import { LOG_PREVIEW_SNIPPET_HEAD_CHARS, LOG_PREVIEW_SNIPPET_MAX } from '../../lib/content-limits.ts';
 ...
 * Catches HTML attribute value case differences, minor reformatting by the model.
 * Fixed window = needleLineCount.
 */
...
/**
 * Anchor on the first and last non-empty lines of oldText; tolerate interior drift.
 * Requires >= 3 lines. Both anchor lines must match uniquely (trimmed + lowercased)
 * and the region between them must be within ±5 lines of the expected size.
 */
 ...
 *
 * When `diagnostics` is provided, each edit's resolution path is recorded for observability.
 */
