...
3. **Layer 3 — What the design LLM sees:** **System message** = **`designer-agentic-system`** (Langfuse / `shared-defaults.ts`) + **`Current date`** and **`Current working directory`** appended by Pi’s `buildSystemPrompt`—those two lines are **not** in Langfuse. **User message** = hypothesis/spec + hardcoded workspace root reminder in [`pi-agent-service.ts`](server/services/pi-agent-service.ts). Langfuse **`agents-md-file`** becomes a **virtual file** `AGENTS.md` in the sandbox (project/output rules); Pi’s `getAgentsFiles()` is empty, so that text is **not** auto-injected into the system prompt—the agent must **`read`** it if you want it in context.
...
| `/api/logs`                     | GET                                       | Dev-only: snapshot `{ llm, trace }` (in-memory rings + optional NDJSON); **not consumed by the SPA** — use tooling or Langfuse                                                                 | JSON                       |
...
**Hypothesis flow:** The canvas still owns graph/domain state (Zustand + React Flow), but **prompt assembly and multi-model orchestration** for a hypothesis go through `/api/hypothesis/*`. Pure workspace helpers live in `src/workspace/hypothesis-generation-pure.ts` (importable by the server). `/api/hypothesis/generate` adds `laneIndex` to each event payload and emits `lane_done` per model lane before a final `done`; the client demuxes into one `GenerationResult` per lane. **Local prompt experimentation (dev SPA + non-production API):** `src/stores/prompt-overrides-store.ts` (Zustand `persist` → `STORAGE_KEYS.PROMPTS`) supplies `**getActivePromptOverrides()`** for API payloads when the UI is a **development build**; `server/lib/prompt-overrides.ts` sanitizes keys and **ignores every override** when `**NODE_ENV=production**`. `**createResolvePromptBody**` layers overrides over `**getPromptBody**` for workspace bundle, incubate, design-system extract, inputs auto-generate, and agentic paths.
...
| `lib/prompt-overrides.ts`                  | Sanitize `**promptOverrides**` (drops all when `**NODE_ENV=production**`); `**createResolvePromptBody**` layers overrides over `**getPromptBody**` in dev/test                                                                                                                                         |
