...
| `routes/logs.ts`                           | GET `/api/logs` → `{ llm, trace, task }`; POST `/api/logs/trace` (Zod); DELETE clears rings (file append-only)                                                                                                                                         |
