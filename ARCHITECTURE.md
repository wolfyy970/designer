...
| `lib/pi-stream-budget.ts`                  | Per-turn `max_tokens` heuristic from prompt-token estimates (consumed by `pi-llm-log.ts`)                                                                                                                                                                |
...
| `services/pi-agent-runtime.ts`             | Single Pi entry point: resolves provider config, dispatches by session type to `@auto-designer/pi` factories, layers the host bridge + LLM log on the returned session, maps `SessionRunResult` → `DesignAgentSessionResult`.                            |
...
| `lib/skill-discovery.ts`                   | Walks `packages/auto-designer-pi/skills/*/SKILL.md`, builds bounded resource manifests for sibling package files, filters by `when` mode, `formatSkillsCatalogXml` / `buildUseSkillToolDescription`; invalid packages omitted (dev `console.warn`)      |
