...
- `**agent-bash-sandbox**` starts empty except optional **caller seeds** (e.g. prior design files on revision rounds); the agent writes generated artifacts only—**skills are not copied into the VFS**. `**extractDesignFiles**` collects the workspace after the run; **`computeDesignFilesBeyondSeed`** compares to the session seed when seeds exist (revision) so unchanged seeded files do not count as new work. Evaluation runs in `**design-evaluation-service**` (parallel workers), not inside Pi tool definitions. Repo-root `**skills/**` packages (see `**server/lib/skill-discovery.ts**`) are walked at each Pi session boundary; non-`**manual**` skills matching the session tags are listed in `**<available_skills>**` on the Pi `**use_skill**` tool (host-backed bodies, not sandbox files). Successful `**use_skill**` calls emit `**skill_activated**` (SSE + trace). `**skills_loaded**` (+ trace) advertises the catalog summary for the UI.
...
| `/api/design-system/extract`    | POST   | Extract design tokens from screenshots (Pi task agent; SSE; final `task_result` with JSON)                                                                                                         | SSE stream                 |
...
| `lib/build-agentic-system-context.ts`      | Composes agentic system context: resolved `**prompts/designer-agentic-system/PROMPT.md`** body + `**skillCatalog`** entries + empty `**sandboxSeedFiles`** (orchestrator may add caller seeds only); skills are **not** copied into the VFS                                            |
| `lib/skill-discovery.ts`                   | Walk `**skills/*/SKILL.md**`, filter by `when` mode, `**formatSkillsCatalogXml**` / `**buildUseSkillToolDescription**`; invalid packages omitted (dev `console.warn` on bad YAML / schema)                                            |
...
| `lib/sse-task-route.ts`                    | Shared SSE wrapper for task-agent routes (`incubate`, `inputs-generate`, `design-system`) — write gate, `error`+`done` on throw; dev write-count summary                                                                                                 |
| `services/task-agent-execution.ts`         | `executeTaskAgentStream` — Pi build-only task sessions for incubation / inputs-gen / design-system; forwards agentic SSE; NDJSON + `log-store` task entries                                                                                              |
...
| File        | Purpose                                                                                                                                                                                                      |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `client.ts` | REST + SSE fetch wrappers. Hypothesis design uses `generateHypothesisStream`; `incubateStream` accepts optional **`IncubateStreamOptions`** (`incubate` vs `agentic` callbacks) so incubation can surface the same Pi agentic events as design runs while still resolving the final `IncubateResponse`. `GenerateStreamCallbacks` includes `onFile` / `onPlan` / trace hooks. SSE frames parsed with Zod in `**src/lib/generate-sse-event-schema.ts**`. |
