...
Optional benchmark harness: a **proposer** proposes edits to **skills**, **rubric weights** (when tools are enabled), and benchmark JSON; a **runner** scores each candidate against your local API. Inspired by [Meta-Harness](https://arxiv.org/abs/2603.28052) (an **outer** optimization loop in the paper’s terms — proposer plus repeated evaluation — not a separate app).

`**--mode=design`** (default) runs fixed strategies through `**POST /api/hypothesis/generate`**. `**--mode=incubate**` runs `**POST /api/incubate**` plus an OpenRouter **hypothesis rubric** (no design build). `**--mode=inputs`** optimizes the three **inputs-generate prompts** (`inputs-gen-research-context`, `inputs-gen-objectives-metrics`, `inputs-gen-design-constraints`) via `**POST /api/inputs/generate`** per test case and a 5-dimension rubric. `**--mode=e2e`** runs the full pipeline: inputs-gen → incubate → random hypothesis → agentic build → multi-rubric eval.
...
| Doc                                   | Use it for                                                                                                                                                                                        |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **This README**                       | What the CLI is, the modes diagram below, copy-paste commands, preflight / promotion behavior at a glance, `history/` layout, version-store note                                                  |
| **[RUNBOOK.md](./RUNBOOK.md)**        | Full operator manual: every `config.json` field, all flags, startup order, iteration lifecycle, tunable surfaces, `PROMOTION_REPORT.md` checklist, troubleshooting, paper-aligned recommendations |
| **[VERSIONING.md](./VERSIONING.md)**  | This CLI and `**.prompt-versions/`** (automatic snapshots from proposer / promotion `**P`**)                                                                                                      |
| **[USER_GUIDE.md](../USER_GUIDE.md)** | **Designer app**: canvas setup/workflow; **§ Version history** — **`pnpm snap`** when you edit prompts/skills/rubric by hand                                                   |


Old bookmarks: [META_HARNESS_OUTER_LOOP.md](./META_HARNESS_OUTER_LOOP.md) redirects to **RUNBOOK.md**.
...
**Version store:** Before `**P`** overwrites live files, the runner copies the **prior** `**skills/`** tree, `**prompts/designer-agentic-system/PROMPT.md`**, and `**src/lib/rubric-weights.json**` contents into **`_versions/`** (skills/PROMPT) and **`.prompt-versions/`** (manifest + rubric snapshots). The **proposer** tools (`write_skill`, `delete_skill`, `write_system_prompt`) do the same. Commit **`_versions/`**, **`.prompt-versions/`**, and skill edits so history travels with the repo. **Details:** **[VERSIONING.md](./VERSIONING.md)**. **Manual** checkpoints (`**pnpm snap**`): **[USER_GUIDE.md § Version history](../USER_GUIDE.md#version-history)**.

Manual promotion checklist: [RUNBOOK.md §5.2](./RUNBOOK.md#52-promotion_reportmd-manual-promotion). **Tunable surfaces** overview: [RUNBOOK.md §3.3](./RUNBOOK.md#33-tunable-surfaces-promotion-model).
