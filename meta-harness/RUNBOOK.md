# Meta-harness CLI — runbook

How to **run and operate** the `pnpm meta-harness` CLI. One **proposer** model (with filesystem tools) proposes harness changes; a **runner** measures them by calling your API and reading `**eval-runs/`** on disk. The design follows the **outer optimization loop** idea from [Meta-Harness](https://arxiv.org/abs/2603.28052) (proposer + repeated evaluate — “outer loop” in the paper’s terms).

**Orientation:** [README.md](./README.md) in this folder has the **modes diagram**, copy-paste commands, preflight summary, and `history/` layout. **[VERSIONING.md](./VERSIONING.md)** explains how this CLI writes **`_versions/`** and **`.prompt-versions/`**. **This file** is the full manual (every `config.json` field, flags, iteration lifecycle, promotion, troubleshooting, and paper-aligned recommendations).
...
The proposer experiments on **skills**, the **designer system PROMPT.md body**, **rubric weights** (design/e2e), and **benchmark JSON**. The live app loads prompt bodies from repo `**skills/*/SKILL.md`** and `**prompts/designer-agentic-system/PROMPT.md`** via `**server/lib/prompt-resolution.ts`**. Preflight `**P**` applies **skills** and **rubric** drift only (see `**apply-promotion.ts`**). Meta-harness snapshots: [VERSIONING.md](./VERSIONING.md). Manual `**pnpm snap**`: **[USER_GUIDE.md § Version history](../USER_GUIDE.md#version-history)**.
...
| **Designer system prompt** | `prompts/designer-agentic-system/PROMPT.md` | `write_system_prompt`         | Session baseline + **`_versions/`** snapshots on write | Not in preflight `**P`** — §5.3; **manual** restore: **[USER_GUIDE.md § Version history](../USER_GUIDE.md#version-history)** | Edit file manually or replay from **`_versions/`**; harness: **[VERSIONING.md](./VERSIONING.md)** |
...
**Overview:** [README.md](./README.md) in this folder.