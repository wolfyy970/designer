...
- **Prompt Studio** — Settings → Prompts: edit prompts against the **server baseline**; **Save** / ⌘S persists drafts **in the browser** and sends **`promptOverrides`** on incubate / generate / extract / **inputs auto-generate** (no automatic save; does not create Langfuse versions from the UI).
