/**
 * Resolve a relative asset ref (href / src) against the directory of the HTML file
 * containing it, returning a normalized key for lookup in a flat virtual-file map
 * (keys like "css/style.css", "pages/about.html").
 *
 * Returns `undefined` for external URLs, data URIs, and protocol-relative refs —
 * callers handle those separately (e.g. the Google Fonts allowlist).
 *
 * Pure function — no Node / browser dependencies.
 */
export function resolveVirtualAssetPath(ref: string, htmlFilePath: string): string | undefined {
...
export function classifyAssetRef(ref: string): 'external' | 'absolute' | 'relative' {
  const clean = ref.split('#')[0]!.split('?')[0]!.trim();
