...
| **USER_GUIDE.md** | Setup, canvas workflow, managing specs; **version-snapshot** CLI for prompt/skill/rubric backups | UX changes |
...
| **meta-harness/README.md** (+ **META_HARNESS_OUTER_LOOP.md**) | Optional benchmark/proposer CLI, config, artifacts | CLI behavior, flags, `history/session-<mode>-*/` layout, promotion report, preflight unpromoted-winner check (diff UI), timeouts / TUI states / proposer strategy, **`.prompt-versions/`** snapshots |
