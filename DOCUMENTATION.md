...
2. **Pi coding-agent adapter** — Legacy path: imports of `@mariozechner/pi-ai` / `@mariozechner/pi-coding-agent`, virtual file tools, and stream/session wiring belong under `server/services/pi-sdk/` (`virtual-tools.ts`, `types.ts`, etc.). Orchestration calls `server/services/agent-runtime.ts`; VFS helpers flow through `virtual-workspace.ts`; tool ordering/grouping flows through `agent-tool-registry.ts`. **In-flight rebuild:** the Pi boundary is moving to `packages/auto-designer-pi/` (own `pi-package` keyword, peer-deps Pi + typebox); see [ARCHITECTURE.md](ARCHITECTURE.md) for the layered phase plan and current cut-over status. New Pi-touching work belongs in the package, not in the legacy path. If you change either boundary, update **ARCHITECTURE.md** (and **SYSTEM_OVERVIEW.md** when the user-visible flow changes) — do not scatter Pi-specific mentions across other docs.
