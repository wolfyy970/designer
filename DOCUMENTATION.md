...
**Documentation serves as persistent memory between human and AI collaborators across context windows.** Well-structured docs enable seamless continuation without loss of critical knowledge. **Assistant sessions do not retain prior conversation**—the handoff path is: [EXPLORATION_LOG.md](EXPLORATION_LOG.md) (pathway backlog + short session notes) → [AGENTS.md](AGENTS.md) → [ARCHITECTURE.md](ARCHITECTURE.md); [README.md](README.md) states this at the top for humans.
...
- **[README.md](README.md)** — Hub and entry point
  ...
  - **[DESIGN_SYSTEM.md](DESIGN_SYSTEM.md)** — SPA design tokens (Indigo palette, light + dark themes): accent vs status, typography triad (body / display / mono / wordmark), atoms (handle, chip, left-rail, buttons, scorecard bar), file-role colors. Visual spec: [AutoDesigner Indigo Reference.html](AutoDesigner%20Indigo%20Reference.html)
  ...
  - **[EXPLORATION_LOG.md](EXPLORATION_LOG.md)** — Pathway backlog and session notes for continuity across context windows (risk-ordered seams; not a second architecture doc)
...
| **DESIGN_SYSTEM.md**                                              | SPA token semantics (`@theme` + `html[data-theme="dark"]`), Indigo palette + sage/amber status, typography triad, atoms (handle/chip/rail/button/scorecard bar), threshold-colored scorecard | New semantic colors/roles, **font stack / wordmark / display** changes, new atoms, token naming, theme changes                                                                                                |
...
| **EXPLORATION_LOG.md**                                              | Pathway exploration backlog + short session notes (continuity across AI/human context windows)                                         | After meaningful passes on a seam, or when discovering a new risk/gap worth tracking                                                                                        |
