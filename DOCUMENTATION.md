...
**After code changes:**
1. Decide which doc owns the fact (see table above). Agentic / Pi / sandbox / **preview session** routes and iframe behavior → **ARCHITECTURE.md** + **SYSTEM_OVERVIEW.md**; product-visible preview + eval UX → **PRODUCT.md** / **USER_GUIDE.md**; UX/setup → **USER_GUIDE.md** or **README.md**; SPA color/type semantics, kitchen sink, or new `@theme` roles → **DESIGN_SYSTEM.md** (values stay in `src/index.css` only). Optional **section ghosts / spec materialization** → **PRODUCT.md** / **USER_GUIDE.md** (behavior), **`spec-materialize-sections`** tests for logic. **Husky patch bump, header version/git timestamp** → **CLAUDE.md** only. Canvas **permanent delete** and **Stop generation** belong in **USER_GUIDE.md** / **PRODUCT.md**; implementation details belong in **ARCHITECTURE.md** / **CLAUDE.md** only. There is **no application database** (Prisma removed); prompts live in **Langfuse**, and **`pnpm db:seed`** runs **`scripts/seed-langfuse.ts`**. Repo-root **`skills/`** (Agent Skills packages) is described in **ARCHITECTURE.md**, **SYSTEM_OVERVIEW.md**, and **PRODUCT.md** — not duplicated here.
